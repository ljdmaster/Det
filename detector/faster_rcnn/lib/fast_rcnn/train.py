...
class SolverWrapper(object):
    ...
    def __init__(self,sess, network, imdb, output_dir, log_dir, pretrained_model=None):
        """Initialize the SolverWrapper."""
        self.net = network
        self.imdb = imdb
        self.roidb = get_roidb(imdb)
    ...
    def snapshot(self, itr):
        ...
        if cfg.TRAIN.BBOX_REG and 'bbox_pred' in net.layers:
            with tf.variable_scope('bbox_pred', reuse=True):
                ...
                self.sess.run(net.bbox_weights_assign, feed_dict={net.bbox_weights: orig_0})
                self.sess.run(net.bbox_bias_assign, feed_dict={net.bbox_biases: orig_1})
    ...
    def train_model(self, max_iters):
        ...
        for itr in range(max_iters):
            ...
            rpn_loss_cls_value, rpn_loss_box_value, loss_cls_value, loss_box_value, summary_str,\
                _ = self.sess.run(fetches=fetch_list, feed_dict=feed_dict, options=run_options, run_metadata=run_metadata)
            ...
            if (itr+1) % cfg.TRAIN.DISPLAY == 0:
                print(
                    'iter: %d / %d, total loss: %.4f, rpn_loss_cls: %.4f, rpn_loss_box: %.4f, loss_cls: %.4f, '
                    'loss_box: %.4f, lr: %f' %
                    (itr + 1, max_iters, rpn_loss_cls_value + rpn_loss_box_value + loss_cls_value + loss_box_value,
                     rpn_loss_cls_value, rpn_loss_box_value, loss_cls_value, loss_box_value, lr))
                print('speed: {:.3f}s / iter'.format(timer.average_time))

            if (itr+1) % cfg.TRAIN.SNAPSHOT_ITERS == 0:
                last_snapshot_iter = itr
                self.snapshot(itr)

        if last_snapshot_iter != itr:
            self.snapshot(itr)
...
def train_net(network, imdb, output_dir, log_dir, pretrained_model=None, max_iters=40000):
    """Train a Fast R-CNN network."""
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        sw = SolverWrapper(sess, network, imdb, output_dir, log_dir, pretrained_model=pretrained_model)
        print('Solving...')
